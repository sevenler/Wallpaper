...
public abstract class BaseListActivity extends BaseActivity {
	...
	private void refreshToLoad(){
		new ImgListUpdateFromTagTask(Const.TAGS.TAG_CALSS_GRIL, IMAGE_SIZE, skip, mLimitEachPage).setOnProgressListenner(new OnProgressListenner() {
			...
			public void onFinish(Object... result) {
				...
				if (LOG.isLoggindAble) LOG.i(BaseListActivity.this, String.format("get %s images:%s",urls.length, Arrays.toString(urls)));
	...
	private class ClassAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (LOG.isLoggindAble) LOG.i(BaseListActivity.this, String.format("loading image:%s", url));
			imageLoader.displayImage(url, image, options, new SimpleImageLoadingListener() {
				@Override
				public void onLoadingComplete(Bitmap loadedImage) {
					Animation anim = AnimationUtils.loadAnimation(BaseListActivity.this, R.anim.fade_in);
	...
	protected abstract GridView initUI();
}
